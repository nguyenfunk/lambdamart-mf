...
public class NDCGEval extends EvaluationMetric {
	/*
	public static final int MAX_TRUNCATION_LEVEL = 35;
	public static final int GAIN_LEVELS = 35;
	public static double[] GAINS = new double[] { 0, 1L, 3L, 7L, 15L, 31L, 63L, 127L, 255L, 511L, 1023L, 2047L, 4095L, 8191L, 16383L, 32767L, 65535L, 131071L, 262143L, 524287L, 1048575L, 2097151L, 4194303L, 8388607L, 16777215L, 33554431L, 67108863L, 134217727L, 268435455L, 536870911L, 1073741823L, 2147483647L, 4294967295L, 8589934591L, 17179869183L };	
	*/
	
	//public static final int MAX_TRUNCATION_LEVEL = 38;
	//public static final int GAIN_LEVELS = 38;
	public static final int MAX_TRUNCATION_LEVEL = 10;
	public static final int GAIN_LEVELS = 10;
	public static double[] GAINS = new double[] { 0, 1, 3, 7, 15, 31 };
	
	public static double[] discounts;

	private TaskCollection<NDCGWorker> ndcgWorkers;
	public int evalTruncationLevel;
	private int maxDocsPerQuery;
	
	// Double array of the ndcg value for each query
	public double[] ndcgs;
	...
	private class NDCGWorker extends TaskItem {
		...
		public void run() {
			...
			for (int q = beginIdx; q < endIdx; q++) {
				...
				double [] localResult = new double[MAX_TRUNCATION_LEVEL]; 
					...
					if (maxDCG[0][sample.queryIndices[q]] == 0) {
						...
						for (int t = 0; t < numDocs; t++) {
							...
							if (dcg > 0) {
								...
								localResult[t] += dcg / maxDCG[t][sample.queryIndices[q]];
							}
						}
						if (dcg > 0) {
							...
							//ndcgs[q] = dcg / maxDCG[evalTruncationLevel-1][sample.queryIndices[q]];
						}
					}
					
					ndcgs[q] = localResult[evalTruncationLevel-1];
	...
	public double[] getNDCGatAllTruncations(double[] predictions, Sample sample, TieBreaker tieBreaker) throws Exception {
		...
		ndcgs = new double[rankingSample.numQueries];
