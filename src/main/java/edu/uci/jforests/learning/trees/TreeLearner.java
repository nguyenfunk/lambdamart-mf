...
import edu.uci.jforests.learning.trees.regression.MultiRegressionTreeSplit;
...
public abstract class TreeLearner extends LearningModule {
	...
	public Ensemble learn(Sample trainSet, Sample validSet) throws Exception {
		...
		trainTreeLeafInstances.init();
		//minInstancesPerLeaf = (int) (curTrainSet.indicesInDataset.length * minInstancePercentagePerLeaf / 100.0);
		minInstancesPerLeaf = (int) (curTrainSet.indicesInDataset.length * minInstancePercentagePerLeaf);
		//System.out.println("MinInstancesPerLeaf="+minInstancesPerLeaf);
		...
		/*
		MultiRegressionTreeSplit rsplit = (MultiRegressionTreeSplit) rootSplit;
		System.out.println(rsplit.leftCount+" "+rsplit.leftMultiOutput);
		System.out.println(rsplit.rightCount+" "+rsplit.rightMultiOutput);
		LeafInstances lil = trainTreeLeafInstances.getLeafInstances(leftChild);
		for(int i=lil.begin; i<lil.end; i++) System.out.print(lil.indices[i]+",");
		System.out.println();
		LeafInstances lir = trainTreeLeafInstances.getLeafInstances(rightChild);
		for(int i=lir.begin; i<lir.end; i++) System.out.print(lir.indices[i]+",");
		System.out.println();
		//System.out.println(trainTreeLeafInstances.getNumberOfInstancesInLeaf(rightChild)+ " "+trainTreeLeafInstances.getNumberOfInstancesInLeaf(leftChild));
		Util.readline();
		*/
		...
		for (int k = 2; k < maxLeaves; k++) {
			...
			/*
			rsplit = (MultiRegressionTreeSplit) bestLeafSplit;
			System.out.println(rsplit.leftCount+" "+rsplit.leftMultiOutput);
			System.out.println(rsplit.rightCount+" "+rsplit.rightMultiOutput);
			lil = trainTreeLeafInstances.getLeafInstances(leftChild);
			for(int i=lil.begin; i<lil.end; i++) System.out.print(lil.indices[i]+",");
			System.out.println();
			lir = trainTreeLeafInstances.getLeafInstances(rightChild);
			for(int i=lir.begin; i<lir.end; i++) System.out.print(lir.indices[i]+",");
			System.out.println();
			//System.out.println(trainTreeLeafInstances.getNumberOfInstancesInLeaf(rightChild)+ " "+trainTreeLeafInstances.getNumberOfInstancesInLeaf(leftChild));
			Util.readline();
