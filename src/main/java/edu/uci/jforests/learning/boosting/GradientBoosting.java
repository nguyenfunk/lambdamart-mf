...
public class GradientBoosting extends LearningModule {
	...
	protected int numSubModules;
	protected double learningRate;
	protected double samplingRate;
	protected double earlyStoppingTolerance;
	protected double earlyStoppingStep;
	...
	public void init(ConfigHolder configHolder, int maxNumTrainInstances, int maxNumValidInstances, EvaluationMetric evaluationMetric) throws Exception {
		...
		this.earlyStoppingStep = gradientBoostingConfig.earlyStoppingStep;
	...
	public Ensemble learn(Sample trainSet, Sample validSet) throws Exception {
		...
		for (curIteration = 1; curIteration <= numSubModules; curIteration++) {
			...
			Sample subLearnerSample = getSubLearnerSample();
			Ensemble subEnsemble = subLearner.learn(subLearnerSample, validSet);
			//System.out.println(subEnsemble);
			//Util.readline();
			...
			for (int t = 0; t < subEnsemble.getNumTrees(); t++) {
				...
				if (validSet != null) {
					updateScores(validSet, tree, validPredictions);
	...
	protected void onLearningEnd() {
		if(curValidSet != null)
			System.out.println("Best validation measure found="+bestValidationMeasurement);
	}
	...
	protected void updateScores(Sample sample, Tree tree,
			double[] scores) {
		LearningUtils.updateScores(sample, scores, ((RegressionTree) tree), 1.0);
	}
