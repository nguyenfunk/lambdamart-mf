...
public class GradientBoostingConfig extends ComponentConfig {
	...
	private final static String EARLY_STOPPING_STEP = "boosting.early.stop.step";
	public double earlyStoppingStep = 100;
	...
	public void init(ConfigHolder config) {
		for (Entry<Object, Object> entry : config.getEntries()) {
			...
			else if (key.equals(EARLY_STOPPING_STEP)) {
				earlyStoppingStep = Double.parseDouble(value);
			}
		}
	}
