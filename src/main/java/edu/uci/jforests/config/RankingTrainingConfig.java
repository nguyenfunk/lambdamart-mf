...
public class RankingTrainingConfig extends TrainingConfig {
	...
	public String inputItemSimilarityFiles = null;
	private final static String RANKING_INPUT_ITEM_SIMILARITY_FILES = "ranking.input.item-similarity-files";
	
	public int inputItemNearestNeighbours = 10;
	private final static String RANKING_INPUT_ITEM_NEAREST_NEIGHBOURS = "ranking.input.item-nearest-neighbours";
	
	public double lambdaInputItem = 0;
	private final static String RANKING_LAMBDA_INPUT_ITEM = "ranking.input.item.lambda";
	
	public String outputItemSimilarityFiles = null;
	private final static String RANKING_OUTPUT_ITEM_SIMILARITY_FILES = "ranking.output.item-similarity-files";
	
	public int outputItemNearestNeighbours = 10;
	private final static String RANKING_OUTPUT_ITEM_NEAREST_NEIGHBOURS = "ranking.output.item-nearest-neighbours";

	public double lambdaOutputItem = 0;
	private final static String RANKING_LAMBDA_OUTPUT_ITEM = "ranking.output.item.lambda";
	...
	public void init(ConfigHolder config) {
		...
		for (Entry<Object, Object> entry : config.getEntries()) {
			...
			} else if (key.equals(RANKING_INPUT_ITEM_SIMILARITY_FILES)) {
				inputItemSimilarityFiles = value;
			} else if (key.equals(RANKING_INPUT_ITEM_NEAREST_NEIGHBOURS)) {
				inputItemNearestNeighbours = Integer.parseInt(value);
			}  else if (key.equals(RANKING_LAMBDA_INPUT_ITEM)) {
				lambdaInputItem = Double.parseDouble(value);
			} else if (key.equals(RANKING_OUTPUT_ITEM_SIMILARITY_FILES)) {
				outputItemSimilarityFiles = value;
			} else if (key.equals(RANKING_OUTPUT_ITEM_NEAREST_NEIGHBOURS)) {
				outputItemNearestNeighbours = Integer.parseInt(value);
			} else if (key.equals(RANKING_LAMBDA_OUTPUT_ITEM)) {
				lambdaOutputItem = Double.parseDouble(value);
			}
		}
	}
